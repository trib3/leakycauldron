...
class DbModule(val configPath: String = "db") : AbstractModule() {
    ...
    @Singleton
    @Provides fun provideDbConfig(): DbConfig {
        return DbConfig(configPath)
    }

    @Provides fun provideDataSource(dbConfig: DbConfig): DataSource {
        return dbConfig.dataSource
    }

    @Provides fun provideDSLContext(dbConfig: DbConfig): DSLContext {
